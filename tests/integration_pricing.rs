...
async fn test_litellm_client_fetch_pricing() {
    ...
    match client.fetch_pricing_data().await {
        Ok(pricing_data) => {
            ...
            let claude_models: Vec<&String> = models
                .iter()
...
async fn test_pricing_manager_enhanced_cost_calculation() {
    ...
    assert!(
        ...
        "Source should be either LiteLLM or StaticFallback, got: {:?}",
        source
    );

    println!(
        "Enhanced cost calculation: ${:.4} (source: {:?})",
        cost, source
    );
}
...
async fn test_static_vs_enhanced_pricing_comparison() {
    ...
    for (model, input, output, cache_creation, cache_read) in test_cases {
        ...
        let (enhanced_cost, source) = enhanced_manager
            .calculate_enhanced_cost(model, input, output, cache_creation, cache_read)
            .await;
...
fn test_pricing_source_configuration() {
    ...
    assert_eq!(
        live_manager.get_pricing_source_info(),
...
async fn test_granular_cache_pricing_accuracy() {
    ...
    let (cost_with_cache, source) = manager
        .calculate_enhanced_cost(
            "claude-sonnet-4-20250514",
            1_000_000, // 1M input tokens
            0,         // No output tokens
    ...
    let (cost_without_cache, _) = manager
        .calculate_enhanced_cost(
            "claude-sonnet-4-20250514",
            1_000_000, // 1M input tokens
            0,         // No output tokens
...
fn test_pricing_configuration_validation() {
    ...
    assert!(
        config
            .set_value("pricing.cache_ttl_minutes", "1440")
            .is_ok()
    );
    ...
    assert!(
        config
            .set_value("pricing.cache_ttl_minutes", "1441")
            .is_err()
    );
    ...
    assert!(
        config
            .set_value("pricing.offline_fallback", "false")
            .is_ok()
    );
    assert!(
        config
            .set_value("pricing.offline_fallback", "invalid")
            .is_err()
    );
}
...
async fn test_pricing_accuracy_scenario() {
    ...
    assert!(
        total_static_cost > 100.0,
        "Total static cost should be substantial"
    );
    assert!(
        total_enhanced_cost > 100.0,
        "Total enhanced cost should be substantial"
    );

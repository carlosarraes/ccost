...
use crate::watch::{Dashboard, DashboardState, FileEvent, FileWatcher, SessionTracker, WatchEvent};
...
impl WatchMode {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        let mut file_event_receiver = std::mem::replace(
        ...
        let mut dashboard_task = tokio::spawn(async move {
            dashboard
                .run_with_events_and_reset(watch_event_receiver, reset_sender)
                .await
        });
    ...
    async fn cleanup(&mut self) {
        ...
        if let Some(task_handle) = self.file_watcher_task.take() {
            ...
            let _ = tokio::time::timeout(std::time::Duration::from_millis(100), task_handle).await;
        }
    }
    ...
    async fn start_file_watcher(&mut self) -> Result<()> {
        ...
        let (file_watcher, file_receiver) =
            FileWatcher::new(self.projects_dir.clone(), self.event_sender.clone())?;
    ...
    async fn process_file_change(&mut self, file_path: PathBuf) -> Result<Vec<WatchEvent>> {
        ...
        let usage_data = self
            .dedup_engine
            .filter_duplicates(parsed_conversation.messages, &project)?;
        ...
        for data in usage_data {
            ...
            let cost = if let Some(cost_usd) = data.cost_usd {
                ...
                if currency == "USD" {
                    ...
                    self.currency_converter
                        .convert_from_usd(cost_usd, currency)
                        .await
                        .unwrap_or(cost_usd)
                }
            } else {
                ...
                if let Some(usage) = &data.usage {
                    ...
                    if currency == "USD" {
                        ...
                        self.currency_converter
                            .convert_from_usd(total_cost_usd, currency)
                            .await
            ...
            let is_new_session =
                self.session_tracker
                    .update_activity(&project, tokens as u32, cost, &model);
            if is_new_session {
            ...
            if let Some(usage) = &data.usage {
                if usage.cache_read_input_tokens.unwrap_or(0) > 0 {
                    ...
                    let saved_cost = (saved_tokens as f64 / 1_000_000.0)
                        * (pricing.input_cost_per_mtok - pricing.cache_cost_per_mtok);
...
mod tests {
    ...
    async fn test_watch_mode_creation() {
        ...
        config.general.claude_projects_path = temp_dir
            .path()
            .join("projects")
            .to_string_lossy()
            .to_string();
    ...
    fn test_unified_project_name_extraction() {
        ...
        let path2 =
            PathBuf::from("/home/user/.claude/projects/-home-user-dir-name/conversation.jsonl");
    ...
    async fn test_project_filter() {
        ...
        config.general.claude_projects_path = temp_dir
            ...
            .to_string();
        let mut watch_mode =
            WatchMode::new(config, Some("test-project".to_string()), 0.10, 200).unwrap();
...
{"uuid":"msg1","requestId":"req1","message":{"content":"Hello","model":"claude-sonnet-4"},"usage":{"inputTokens":10,"outputTokens":20},"costUSD":0.001,"cwd":"/home/user/test-project","originalCwd":"/home/user/test-project"}"#;
    ...
    async fn test_session_cost_reset_on_new_watch_mode() {
        ...
        let _ = watch_mode1
            .process_file_change(test_file.clone())
            .await
            .unwrap();
        ...
        assert!(
            session_cost1 > 0.0,
            "First session should have non-zero cost, got {}",
            session_cost1
        );
        ...
        assert_eq!(
            session_cost2, 0.0,
            "New watch mode session should start at $0, got {}",
            session_cost2
        );
    }
    ...
    async fn test_reset_file_tracking_resets_sessions() {
        ...
        assert!(
            session_cost_before > 0.0,
            "Session should have activity before reset"
        );
        ...
        assert_eq!(
            session_cost_after, 0.0,
            "Sessions should be reset to $0 after reset_file_tracking"
        );
    }
...
{"uuid":"msg2","requestId":"req2","message":{"content":"World","model":"claude-sonnet-4"},"usage":{"inputTokens":150,"outputTokens":250},"costUSD":0.15,"cwd":"/home/user/test-project","originalCwd":"/home/user/test-project"}"#;
        ...
        // Second session (restart)
        {
            ...
            assert!(
                session2_cost > 0.0,
                "Session 2 should accumulate costs after processing"
            );
        }
    ...
    fn test_reset_sessions_method() {
        ...
        watch_mode
            .session_tracker
            .update_activity("test-project", 1000, 0.5, "claude-3-sonnet");
    ...
    async fn test_file_watcher_task_cleanup() {
        ...
        assert!(
            watch_mode.file_watcher_task.is_some(),
            "File watcher task handle should be stored"
        );
        ...
        assert!(
            watch_mode.file_watcher_task.is_none(),
            "File watcher task handle should be removed after cleanup"
        );
    }
    ...
    fn test_drop_cleanup() {
        ...
        assert!(
            watch_mode.file_watcher_task.is_some(),
            "Task should be present before drop"
        );
    ...
    fn test_zero_token_filtering_logic() {
        ...
        assert_eq!(
            ...
            2,
            "Should filter out zero-token messages"
        );

        // Verify correct messages are kept
        let kept_ids: Vec<_> = filtered_messages
            .iter()
            .map(|m| m.uuid.as_ref().unwrap().as_str())
            .collect();
        assert!(
            kept_ids.contains(&"msg2"),
            "Should keep message with input/output tokens"
        );
        assert!(
            kept_ids.contains(&"msg4"),
            "Should keep message with cache creation tokens"
        );
        assert!(
            !kept_ids.contains(&"msg1"),
            "Should filter out zero-token message"
        );
    }
    ...
    fn test_should_display_message_logic() {
        ...
        assert!(
            !should_display_message_standalone(&zero_token_data),
            "Zero token message should be filtered"
        );
        assert!(
            should_display_message_standalone(&input_token_data),
            "Message with input tokens should be displayed"
        );
        assert!(
            should_display_message_standalone(&cache_token_data),
            "Message with cache tokens should be displayed"
        );
        assert!(
            !should_display_message_standalone(&no_usage_data),
            "Message with no usage data should be filtered"
        );
    }
    ...
    fn test_filtering_preserves_cache_read_tokens() {
        ...
        assert!(
            should_display_message_standalone(&cache_read_msg),
            "Should preserve messages with cache read tokens"
        );
    }
}

...
impl SessionTracker {
    ...
    pub fn get_session_statistics(&self) -> SessionStatistics {
        let active_sessions: Vec<_> = self.sessions.values().filter(|s| s.is_active).collect();
        ...
        let most_active_project = active_sessions
            .iter()
            .max_by_key(|s| s.message_count)
            .map(|s| s.project.clone());

        let most_expensive_project = active_sessions
            .iter()
            .max_by(|a, b| {
                a.total_cost

...
pub async fn handle_this_week_command(
    ...
    let (final_project, final_since, final_until, final_model) =
        resolve_filters(Some(UsageTimeframe::ThisWeek), project, since, until, model, &context.timezone_calc);

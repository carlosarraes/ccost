    ...
    match action {
        ConfigAction::Init => match Config::default().save() {
            Ok(()) => {
                ...
                } else if let Ok(config_path) = Config::default_path() {
            ...
            Err(e) => {
                if json_output {
                    println!(
                        r#"{{"status": "error", "message": "Failed to initialize config: {e}"}}"#
                    );
                } else {
            ...
            Ok(mut config) => match config.set_value(&key, &value) {
                Ok(()) => match config.save() {
                    Ok(()) => {
                        if json_output {
                            println!(
                                r#"{{"status": "success", "message": "Configuration updated: {key} = {value}"}}"#
                            );
                        } else {
                    ...
                    Err(e) => {
                        if json_output {
                            println!(
                                r#"{{"status": "error", "message": "Failed to save config: {e}"}}"#
                            );
                        } else {
                ...
                Err(e) => {
                    if json_output {
                        println!(
                            r#"{{"status": "error", "message": "Invalid configuration: {e}"}}"#
                        );
                    } else {
